...
const stories = [
  ...
  { ids: ["FEDFUNDS", "DGS10", "DGS2"], horizon: 1825, title: "The yield curve records the policy handoff.", copy: ([fed, ten, two]) => { const twoValue = two.observations.at(-1)[1]; const tenValue = ten.observations.at(-1)[1]; return `The policy rate stands at <span class="story-stat">${format(fed.observations.at(-1)[1], "%")}</span>, versus <span class="story-stat">${format(twoValue, "%")}</span> at two years and <span class="story-stat">${format(tenValue, "%")}</span> at ten years. The 10Y–2Y slope is <span class="story-stat">${signed((tenValue - twoValue) * 100)} bp</span>, a more useful comparison than placing a rising stock-price level beside mean-reverting rates.`; }, measure: "level" },
...
for (const [index, story] of stories.entries()) {
  ...
  const caption = `${seriesList.map((series) => series.id).join(" · ")} · ${story.measure === "indexed" ? "first visible observation = 100" : story.measure === "yoy" ? "year-over-year percent change" : "reported level"} · latest releases ${dates.join(" / ")} · FRED`;
  article.innerHTML = `<div class="story-copy"><div class="story-number">0${index + 1}</div><h2>${story.title}</h2><p>${story.copy(seriesList)}</p></div><figure class="story-chart"><canvas aria-label="${story.title}"></canvas><figcaption>${caption}</figcaption></figure>`;
